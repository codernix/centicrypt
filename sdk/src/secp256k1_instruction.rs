...
//! [`ecrecover`] precompile. For that Centicrypt provides the [`secp256k1_recover`]
//! syscall.
//!
...
//! and callers should ensure they use the same version of `bincode` as the Centicrypt SDK.
//! This data structure is not provided to Centicrypt programs,
...
//! does prevent malleability by default. Centicrypt accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a Centicrypt program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a Centicrypt program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Centicrypt program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Centicrypt program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//!     // Centicrypt does not do this itself.
...
//! containing many signatures, and a Centicrypt program that parses them all. This
...
//! transaction. In the _previous_ example, the Centicrypt program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Centicrypt program supports signatures, messages, and
...
///
/// `feature_set` is the set of active Centicrypt features. It is used to enable or

...
account is needed if you plan to run a validator node on Centicrypt.
...
the validator operator may choose to store only enough CNTC on the identity
...
a vote account withdraws all remaining CNTC funds to a supplied recipient address
